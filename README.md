...
and the variable before stores the previous value of current. When object is not detected the sensor value would be zero and when the object is detected the sensor value 
would be one. So, suddenly when a new body is introduced in front of the sensor, current value would be one and the previous value would be zero(since there was no object in 
...
incremented by one. Our next task is to print this value of count in the LCD display. For that I reffered to the example code given in arduino IDE and modified it to suit my 
curent purpose.
